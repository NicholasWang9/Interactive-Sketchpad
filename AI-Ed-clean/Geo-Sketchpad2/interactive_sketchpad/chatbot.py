...
GEOMETRY_TOOL = {
    ...
    "description": (
        "Generate a geometry diagram image (PNG) from a series-parallel topology string "
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
async def start_chat():
    ...
    await cl.Message(content=f"DEBUG tools = {[t.get('type') for t in [CODE_INTERPRETER_TOOL, GEOMETRY_TOOL, LATEX_TOOL]]}").send()
...
async def main(message: cl.Message):
    ...
    # Decide active tool set (keep both GEOMETRY_TOOL and LATEX_TOOL available, plus code interpreter)
    tools = [CODE_INTERPRETER_TOOL, GEOMETRY_TOOL, LATEX_TOOL]
