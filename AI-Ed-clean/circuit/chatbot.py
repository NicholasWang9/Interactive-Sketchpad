...
from circuit_generation import generate as generate_circuit_png
...
CIRCUIT_TOOL = {
    ...
    "function": {
        ...
        "description": (
            ...
            "using R, +, //, and parentheses. Example: '(R//(R+(R//R)))'."
        ),
        "parameters": {
            "type": "object",
            "properties": {
...
class EventHandler(AsyncAssistantEventHandler):
    ...
    async def on_tool_call_done(self, tool_call):
        ...
        # Call your in-process generator (returns PNG bytes)
        png_bytes = generate_circuit_png(topology, dpi=dpi, pretty=pretty)

        # Send to the whiteboard (you already have this)
        await send_image_to_canvas(png_bytes)

        # Submit tool outputs so the model continues the run
        thread_id = cl.user_session.get("thread_id")
        run_id = cl.user_session.get("run_id")

        if not thread_id or not run_id:
            raise RuntimeError(
                f"Missing thread_id/run_id (thread_id={thread_id}, run_id={run_id}). "
                "Make sure start_chat sets thread_id and EventHandler.on_run_created stores run_id."
            )
            ...
            tool_outputs=[
                {
                    ...
                    "output": "Circuit generated and sent to canvas.",
                }
            ],
        )      
