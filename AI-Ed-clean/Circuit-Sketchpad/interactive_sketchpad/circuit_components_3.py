...
_DIRECTIVE_RE = re.compile(
    r"^\s*(?P<mode>partial|loop)\s*(?:\[(?P<opts>[^\]]*)\])?\s*:\s*(?P<expr>.*)\s*$",
    re.IGNORECASE,
)

def _parse_topology_directives(topology: str) -> Tuple[str, str, float, bool, str, str, Optional[str]]:
    """
    Parse optional render directives embedded in the topology string.

    Supported prefixes:
      - partial:(<expr>)
      - partial[a,b]:(<expr>)
      - partial[lead=1.2,a,b]:(<expr>)
      - loop:(<expr>)  (explicit; default is loop when no directive)
      - loop[<battery_label>]:(<expr>)  (label can be any text; used to label the battery)

    Returns:
      (expr, mode, lead, port_labels, left_port, right_port, battery_label)
    """
    s = topology.strip()
    m = _DIRECTIVE_RE.match(s)
    if not m:
        return topology, "loop", 0.8, False, "in", "out", None
    ...
    lead = 0.8
    port_labels = False
    left_port, right_port = "in", "out"
    battery_label: Optional[str] = None

    opts = (m.group("opts") or "").strip()
    if opts:
        if mode == "loop":
            # In loop mode, treat the entire bracket content as an (optional) battery label.
            # Format: loop[<label>]:(<expr>)
            # Label can be any text except ']'.
            battery_label = opts.strip() or None
        else:
            # partial mode: parse comma-separated options
            parts = [p.strip() for p in opts.split(",") if p.strip()]
            ports: List[str] = []
            for p in parts:
                if p.lower().startswith("lead="):
                    try:
                        lead = float(p.split("=", 1)[1])
                    except ValueError:
                        raise TopologyParseError(f"Bad lead value in directive: {p!r}")
                else:
                    ports.append(p)
    ...
    return expr, mode, lead, port_labels, left_port, right_port, battery_label
...
def latexify_simple_math_token(tok: str) -> str:
    """Latexify a short math token that may include an underscore subscript.

    Intended for port labels like "in", "out", "v_in", "a", "b".
    Braces everything after the first underscore so multi-character subscripts
    render correctly (e.g., v_in -> v_{in}, v_out_1 -> v_{out_1}).

    Keep port names simple (letters/digits/underscores) for best results.
    """
    return latexify_symbol(tok) if "_" in tok else tok


def brace_subscripts_math(s: str) -> str:
    """Brace multi-character subscripts in a LaTeX math-ish string.

    Turns occurrences like:
      V_in -> V_{in}
      v_out_1 -> v_{out_1}
    Leaves already-braced subscripts (_{...}) unchanged.

    This is intentionally lightweight and aims to improve rendering, not validate LaTeX.
    """
    # Replace _<alnum/underscore+> with _{...} unless already _{
    def repl(m: re.Match) -> str:
        head = m.group(1)
        tail = m.group(2)
        return f"{head}_{{{tail}}}"
    # head: letters/digits; tail: one or more letters/digits/underscores
    return re.sub(r"(?<!_)\b([A-Za-z0-9]+)_([A-Za-z0-9_]+)", repl, s)


_LABEL_TOKEN_RE = re.compile(r"([RCL])_(\{[^}]*\}|[A-Za-z0-9_]+)")

def latexify_topology_math(s: str) -> str:
    """Make topology strings render nicely in $...$ by bracing multi-char subscripts.
    ...
    def repl(m):
        ...
        if tail.startswith('{') and tail.endswith('}'):
            return f"{head}_{tail}"
        return f"{head}_{{{tail}}}"
    return _LABEL_TOKEN_RE.sub(repl, s)
...
def make_tex_document(
    ...
    *,
    show_label: bool,
    mode: str = "loop",             # "loop" (battery + loop) or "partial" (2-terminal subcircuit)
    lead: float = 0.8,              # lead wire length (cm) on left/right in partial mode
    port_labels: bool = False,      # optional labels at the terminals in partial mode
    left_port: str = "in",
    right_port: str = "out",
    battery_label: Optional[str] = None,
) -> str:
    ...
    # loop-mode bottom wire y
    y_bottom = -(h / 2 + 2.5)

    # label inside picture, above the network
    y_label = (h / 2 + 1.2)
    ...
    if mode == "loop":
        batt_opt = ""
        if battery_label:
            batt_tex = brace_subscripts_math(battery_label)
            batt_opt = rf",l={{{{${batt_tex}$}}}}"
        body = rf"""
{label_node}
\draw (-0.8,{y_bottom:.3f}) to[battery{batt_opt}] (-0.8,0);
\draw (-0.8,0) -- (0,0);
{circuit_snippet}
...
"""
        ...
        xL = -float(lead)
        xR = float(w) + float(lead)
        port_nodes = ""
        if port_labels:
            lp = latexify_simple_math_token(left_port)
            rp = latexify_simple_math_token(right_port)
            port_nodes = rf"""
\node[anchor=east] at ({xL:.3f},0) {{$ {lp} $}};
\node[anchor=west] at ({xR:.3f},0) {{$ {rp} $}};
"""
        body = rf"""
{label_node}
\draw ({xL:.3f},0) -- (0,0);
...
def _render_one_to_png_bytes(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="circuitgen_") as td:
        ...
        title = f"Circuit 1: $ {latexify_topology_math(topo)} $"

        tex_doc_unlabeled = make_tex_document(
            d.tex, d.w, d.h, title=title,
            show_label=False,
            mode=mode, lead=lead,
            port_labels=port_labels, left_port=left_port, right_port=right_port,
            battery_label=battery_label,
        )
        tex_path.write_text(tex_doc_unlabeled, encoding="utf-8")

        tex_doc_labeled = make_tex_document(
            d.tex, d.w, d.h, title=title,
            show_label=True,
            mode=mode, lead=lead,
            port_labels=port_labels, left_port=left_port, right_port=right_port,
...
def generate(topology: str, *, dpi: int = 300, pretty: bool = True) -> bytes:
    ...
    Supports optional render directives embedded in the topology string:
      - partial:(<expr>)
      - partial[a,b]:(<expr>)
      - partial[lead=1.2,a,b]:(<expr>)
      - loop:(<expr>)   (explicit; default is loop when no directive)
      - loop[<battery_label>]:(<expr>)   (battery label)
    """
    expr_s, mode, lead, port_labels, left_port, right_port, battery_label = _parse_topology_directives(topology)
    e = parse_topology(expr_s)
    return _render_one_to_png_bytes(
        e,
        dpi=dpi,
        pretty=pretty,
        mode=mode,
...
def main():
    ...
    mode, lead, port_labels, left_port, right_port, battery_label = "loop", 0.8, False, "in", "out", None

    if args.topology:
        expr_s, mode, lead, port_labels, left_port, right_port, battery_label = _parse_topology_directives(args.topology)
        e = parse_topology(expr_s)
        items = [(1, e)]
    ...
    for i, e in items:
        ...
        title = f"Circuit {i}: $ {latexify_topology_math(topo)} $"
