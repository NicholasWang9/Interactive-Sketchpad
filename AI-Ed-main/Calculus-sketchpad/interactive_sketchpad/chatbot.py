...
You are a professioanl calculus tutor. Your goal is to guide student to solving problems
...
async def send_image_to_canvas(image_bytes: bytes):
    ...
    # don't send to canvas because on the server there is no canvas :)
...
async def main(message: cl.Message):
    ...
    async with async_openai_client.responses.stream(
        ...
        async for event in stream:
            ...
            # code interpreter artifacts ar eadded as "annotations"
            # which need to be downloaded manually - https://developers.openai.com/api/docs/guides/tools-code-interpreter
                ...
                image_bytes = ( 
                    ...
                    .files
                    .content
                    .retrieve(file_id=file_id, container_id=container_id)
                ).read()
                await send_image_to_canvas(image_bytes)
