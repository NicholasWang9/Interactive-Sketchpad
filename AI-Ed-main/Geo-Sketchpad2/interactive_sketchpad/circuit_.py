...
# ============================================================
# Series–parallel model (R can be unlabeled or have a value)
#   - R      : unlabeled
#   - 1R     : labeled (1 Ω)
#   - 4.7R   : labeled (4.7 Ω)
#   - 3/2R   : labeled (1.5 Ω)  [fraction syntax supported]
# ============================================================

def R(val: Optional[float] = None) -> Dict[str, Any]:
    return {"t": "R", "val": val}
...
def _fmt_r_val(v: float) -> str:
    """Format resistor value for topology/label: 1.0->'1', 4.70->'4.7'."""
...
def expr(e: Dict[str, Any]) -> str:
    ...
    if e["t"] == "R":
        v = e.get("val", None)
        return "R" if v is None else f"{_fmt_r_val(v)}R"
    if e["t"] == "S":
        return f"({expr(e['a'])}+{expr(e['b'])})"
    return f"({expr(e['a'])}//{expr(e['b'])})"

def key(e: Dict[str, Any]) -> str:
    """Canonical key. Series ordered, parallel commutative. Includes R values if present."""
    if e["t"] == "R":
        v = e.get("val", None)
        return "R" if v is None else f"R[{_fmt_r_val(v)}]"
...
def tikz_ohms_label(v: float) -> str:
    """Circuitikz label for ohms: 1 -> '$1\\,\\Omega$' ; 4.7 -> '$4.7\\,\\Omega$'."""
    return rf"$ {_fmt_r_val(v)}\,\Omega $"
...
# Tokens:
#  - operators: //, +, (, )
#  - resistor token: R or <number>R where number is:
#       - decimal/int:   12, 4.7
#       - fraction:      3/2, 10/3
_TOKEN_RE = re.compile(r"\s*(//|\+|\(|\)|(?:(?:\d+(?:\.\d+)?)|(?:\d+/\d+))?R)\s*")
...
def _parse_value_prefix(prefix: str) -> float:
    """Parse decimal/int or fraction like '3/2'."""
    if "/" in prefix:
        num_s, den_s = prefix.split("/", 1)
        num = float(num_s)
        den = float(den_s)
        if den == 0:
...
def parse_topology(s: str) -> Dict[str, Any]:
    ...
    def parse_term() -> Dict[str, Any]:
        ...
        if tok.endswith("R"):
            ...
            prefix = tok[:-1]
            if prefix == "":
                return R(None)  # unlabeled
            try:
                return R(_parse_value_prefix(prefix))
            except ValueError:
                raise TopologyParseError(f"Bad resistor token {tok!r}")
        if tok == "(":
            consume("(")
            node = parse_expr()
            consume(")")
            return node
        raise TopologyParseError(f"Expected resistor token or '(', got {tok!r}")
...
def draw(e: Dict[str, Any], pretty: bool = False) -> DrawOut:
    ...
    if e["t"] == "R":
        ...
        if v is None:
            ...
            body = rf"\draw (0,0) -- (0.4,0) to[R,l={{{tikz_ohms_label(v)}}}] (1.6,0)--(2,0);"
...
def main():
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Explicit topology. R is unlabeled; 1R/4.7R/3/2R are labeled. Example: '(R//(1R+(3/2R//R)))'"
    )
    args = ap.parse_args()
